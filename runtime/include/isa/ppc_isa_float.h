...
inline PpcPairVec PpcNegateNonNanLanesInline(PpcPairVec value)
{
    ...
    return vbsl_f32(vceq_f32(value, value), negated, value);
#endif
}
...
{
    // Lane 0 becomes ps1 and lane 1 becomes ps0, matching the union layout bit for bit.
#if defined(__x86_64__)
    return PpcM128ToPsInline(_mm_unpacklo_ps(_mm_set_ss(ps1), _mm_set_ss(ps0)));
#else
    return PpcM128ToPsInline(vset_lane_f32(ps0, vdup_n_f32(ps1), 1));
#endif
}
...
inline PpcPairVec PpcFmaddPairInline(PpcPairVec lhs, PpcPairVec rhs, PpcPairVec addend)
{
#if defined(__x86_64__)
    return _mm_fmadd_ps(lhs, rhs, addend);
#else
    return vfma_f32(addend, lhs, rhs);
#endif
}

inline PpcPairVec PpcFmsubPairInline(PpcPairVec lhs, PpcPairVec rhs, PpcPairVec subtractor)
{
#if defined(__x86_64__)
    return _mm_fmsub_ps(lhs, rhs, subtractor);
#else
    return vfma_f32(vneg_f32(subtractor), lhs, rhs);
#endif
}
